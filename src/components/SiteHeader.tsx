      ...
      <div className="mx-auto flex h-16 max-w-6xl items-center px-4">
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          <button
            ...
            aria-expanded={open}
            onClick={() => setOpen((v) => !v)}
            className="grid h-11 w-11 place-items-center rounded-lg text-slate-700 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-600"
          >
          ...
          <nav className="absolute right-2 top-[60px] z-30 w-56 overflow-hidden rounded-xl border border-slate-200 bg-white py-1 shadow-lg">
            {HEADER_MENU.map((m) => (
              <a
                ...
                className="block px-4 py-3 text-sm text-slate-700 hover:bg-slate-50 hover:text-teal-700 focus-visible:bg-slate-50 focus-visible:outline-none"
              >
                {m.label}
              </a>
            ))}
