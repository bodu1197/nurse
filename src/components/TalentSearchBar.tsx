...
// 인재 검색바 — /jobs 의 JobSearchBar 와 **같은 모양·같은 조작법**(오너 지시 2026-07-28 "일관성 유지").
// 지역 RegionPicker(도>시군구, 인재 수) + 키워드 + 검색. 지역 목록은 서버가 준 계단을 그대로 쓴다.
//
// 지역만 고르는 옛 TalentRegionBar 를 대신한다 — 키워드 입력이 생기면서, 지역을 고를 때 타이핑 중인
// 키워드를 함께 실어 보내야 하기 때문이다(따로 두면 지역을 누르는 순간 입력이 날아간다).
...
export default function TalentSearchBar({
  sidos,
  sigungus,
}: Readonly<{ sidos: TalentRegionNode[]; sigungus: TalentRegionNode[] }>) {
  ...
  // "무엇"(키워드)과 "어디"(지역)를 함께 실어 이동 — 지금 걸린 다른 필터(dept/cat/years)는 유지한다.
  ...
  return (
    <form
      onSubmit={(e) => { e.preventDefault(); go(q, sido, sigungu); }}
      role="search"
    ...
    >
      <RegionPicker
        ...
        loading={pending}
        onPick={(level, value) => {
          // 시도를 바꾸면 하위 시군구는 다른 도의 것이라 무효 → 함께 비운다(캐스케이드).
