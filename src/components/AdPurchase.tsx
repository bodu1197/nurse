...
export default function AdPurchase({ jobId, impCode, pg }: Readonly<{ jobId: string; impCode: string; pg: string }>) {
  ...
  return (
    <div className="mt-6">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        {AD_PRODUCTS.map((p) => (
          <label key={p.weeks}>
            ...
            <span className="flex h-20 cursor-pointer flex-col items-center justify-center gap-0.5 rounded-[12px] border border-slate-300 text-sm text-slate-600 peer-checked:border-teal-500 peer-checked:bg-teal-50 peer-checked:text-teal-700">
              <b>{p.weeks}주</b>
              <span className="text-xs">{won(p.amount)}</span>
              {p.discount > 0 && <span className="text-[11px] font-semibold text-teal-600">{Math.round(p.discount * 100)}% 할인</span>}
            </span>
          </label>
        ))}
      </div>
      ...
      <p className="mt-2 text-center text-xs text-slate-400">광고 기간 동안 공고가 상단에 노출됩니다. 카드 결제(이니시스).</p>
    </div>
  );
}
