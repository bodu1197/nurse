    ...
    <>
      ...
      <div className="mx-auto flex w-full max-w-[1280px] flex-1 flex-col gap-5 px-4 py-6 lg:flex-row lg:gap-6">
          ...
          <nav aria-label="마이페이지" className="mt-3 space-y-1 rounded-2xl border border-slate-200 bg-white p-2">
            {NAV.map((n) => (
              <a key={n.href} href={n.href} aria-current={n.href === active ? "page" : undefined}
                className={`block rounded-lg px-3 py-2 text-base ${n.href === active ? "bg-teal-50 font-semibold text-teal-700" : "text-slate-600 hover:bg-slate-50"}`}>{n.label}</a>
            ))}
          </nav>
        </aside>
        ...
        <nav aria-label="마이페이지" className="-mx-4 flex gap-2 overflow-x-auto px-4 pb-1 lg:hidden">
          {NAV.map((n) => (
            <a key={n.href} href={n.href} aria-current={n.href === active ? "page" : undefined}
              className={`shrink-0 rounded-full border px-3 py-1.5 text-base ${n.href === active ? "border-teal-500 bg-teal-50 font-semibold text-teal-700" : "border-slate-300 text-slate-600"}`}>{n.label}</a>
          ))}
        </nav>
