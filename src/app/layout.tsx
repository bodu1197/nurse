...
export default function RootLayout({
  ...
  return (
    <html lang="ko" className="h-full antialiased">
      {/* pb-14: 하단 탭(SiteHeader)이 화면 아래에 고정돼 있어, 그만큼 비워두지 않으면
          목록 마지막 줄과 「더 보기」 버튼이 탭 밑에 깔려 손이 닿지 않는다.
          🔴 기준(lg)은 SiteHeader 의 하단 탭 `lg:hidden` 과 **반드시 같아야 한다** — 어긋나면
             그 구간에서 탭이 본문을 덮거나(좁게 잡으면) 빈 띠가 남는다(넓게 잡으면). */}
      <body className="min-h-full flex flex-col bg-white pb-14 text-slate-900 lg:pb-0">
