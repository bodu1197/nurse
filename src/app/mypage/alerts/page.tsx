            ...
            {searches.map((s) => (
              <li key={s.id} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white p-4">
                ...
                <form action={deleteSavedSearch} className="inline">
                  ...
                  <button type="submit" className="shrink-0 rounded-md px-3 py-1.5 text-xs font-semibold text-red-600 hover:bg-red-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500">삭제</button>
                </form>
              </li>
            ))}
          </ul>
