...
function StatusButton({ id, status, label, cls }: { id: string; status: string; label: string; cls: string }) {
  return (
    <form action={updateApplicationStatus} className="inline">
      ...
      <button type="submit" className={`rounded-md px-3 py-1.5 text-xs font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-600 ${cls}`}>{label}</button>
    </form>
  );
}
