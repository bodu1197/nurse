...
export default async function AdOrdersPage() {
  ...
  return (
    <HospitalShell displayName={p.displayName} active="/mypage/jobs">
      ...
      {error && (
        <div role="alert" className="mt-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-base text-red-700">
          ...
          {orders.map((o) => {
            ...
            return (
              <li key={o.id} className="flex flex-wrap items-center gap-x-3 gap-y-2 rounded-xl border border-slate-200 bg-white p-4">
                ...
                <span className="shrink-0 text-sm text-slate-500">{o.days}일</span>
                <span className="shrink-0 text-sm text-slate-500">{fmt(o.paid_at ?? o.created_at)}</span>
                ...
                <span className="shrink-0 text-right">
                  ...
                  {o.cash_used > 0 && <span className="block text-sm text-slate-500">캐시 {won(o.cash_used)} 사용</span>}
                </span>
