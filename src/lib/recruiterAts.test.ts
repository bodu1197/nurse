...
/**
 * 🔴 일부 병원은 상세를 자바스크립트로 그린다 — 받아 온 HTML 에 공고 내용이 아예 없고
 *    목록 UI 만 남는다. 그걸 본문으로 담아 카드에 "10개씩 보기 … 검색" 이 찍혔다
 *    (실측 2026-08-12, 중앙대광명·용인세브란스 43자).
 */
test("목록 UI 만 남은 페이지는 본문으로 치지 않는다", () => {
  const spa = `<html><body><ul><li>10개씩 보기</li><li>20개씩 보기</li><li>30개씩 보기</li>
    <li>50개씩 보기</li><li>100개씩 보기</li></ul><button>검색</button></body></html>`;
  assert.equal(extractBody(spa), null);
});

/** 반대로 알맹이 표시가 없어도 **긴 글**은 공고문일 수 있다 — 버리면 본문을 잃는다. */
test("표시가 없어도 충분히 긴 글은 본문으로 쓴다", () => {
  const long = `<body><p>${"간호사 모집 안내입니다. ".repeat(30)}</p></body>`;
  assert.ok((extractBody(long) ?? "").length > 300);
});
