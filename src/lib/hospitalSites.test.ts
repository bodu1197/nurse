...
/**
 * 🔴 본문 규칙에 **끝 마커가 없으면 푸터를 통째로 삼킨다.** 시작만 잡고 길이로 열어 두면
 *    (`([\s\S]{0,60000})`) 본문이 짧은 공고에서 진료과 메뉴·사업자등록번호까지 딸려온다
 *    — 실제로 건국대병원 공고에서 그랬다(오너 신고 2026-08-12).
 *    본문이 길 때는 우연히 안 보이므로, 사람 눈이 아니라 규칙으로 막는다.
 */
test("detailBody 는 끝 마커로 닫는다 — 길이로 열어 두지 않는다", () => {
  for (const s of SITES) {
    if (!s.detailBody) continue;
    // 🔴 "게으른 수량자 **바로 뒤에** 리터럴 태그" 를 한 번에 강제한다. 두 조건으로 나누면
    //    캡처와 무관한 `)` 로 통과하는 규칙이 생긴다(예: `/(?:x)<div>([\s\S]*?)/` — 늘 빈 문자열).
    assert.match(
      s.detailBody.source,
      /\[\\s\\S\][*+]\??\)?</,
      `${s.key}: 본문 캡처를 끝 마커로 닫아야 한다(([\\s\\S]*?)<끝> 형태) — ${s.detailBody.source}`,
    );
  }
});
