/**
 * 구 널스넷 인재정보 중 **이관에서 빠진 992건** → resumes + work_experiences. 1회성 스크립트.
 *
 * 원본 표(wp_bhjob_person_item)에서 뽑아 resumes_legacy_missing.json 으로 변환한 뒤 돌린다.
 * 매핑은 import-legacy-resumes.ts 와 같다 — 같은 규칙으로 들어가야 먼저 온 7,263건과 모양이 안 갈린다.
 *
...
import { chunk, restHeaders, loadLegacyProfileMap, fetchAllPages, htmlToText } from "./_legacy-util.ts";
// 🔴 이름 규칙(20260804210000)이 DB CHECK 로 걸려 있다 — "ㄱ김선영" 처럼 문장부호·자모로 시작하거나
//    이모지가 든 이름은 **적재 자체가 거부된다**. 같은 규칙으로 미리 정리해서 넣는다.
...
const raw: string[][] = ["resumes_legacy_missing.json"]
...
// htmlToText 는 공용본(_legacy-util)을 쓴다 — 옛 버전은 태그 주변 개행을 안 지워 줄마다 빈 줄이 낀다.
...
for (const r of rows) {
  ...
  resumes.push({
    profile_id: pid,
    name: cleanPersonName(nameOf(r) ?? "") ?? null,
